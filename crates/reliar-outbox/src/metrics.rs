...
pub trait OutboxMetrics: Send + Sync {
    ...
    /// Called once per envelope successfully enqueued through [`crate::OutboxPublisher::enqueue`]
    /// or [`crate::OutboxPublisher::enqueue_batch`], with `n = 1` — the same call shape as
