...
- `Dockerfile.migrate` — a one-shot migration-runner image built from
  `reliar-store-postgres`'s `examples/migrate.rs` (which calls the crate's public `migrate()`
  API) for teams that run migrations through their own pipeline.
